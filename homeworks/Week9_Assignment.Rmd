...
```{r}
# 2d
newsurveys4 <- left_join(surveys, species) %>% left_join(plots) %>%
filter(genus == "Dipodomys") %>%
select(year, genus, species, weight, plot_type)
write_csv(newsurveys4, "../data_clean/newsurveys.csv")
# 2e
ggplot(surveys, aes(weight, hindfoot_length, color = species_id)) + geom_point() +
scale_x_log10() +
labs(x = "Weight (g)",
       y = "Hindfoot Length (mm)")
ggsave("../outputs/surveys1.jpg")
# 2f
newsurveys5 <- surveys %>% filter(!is.na(weight)) 
ggplot(newsurveys5, aes(weight)) +
geom_histogram() +
facet_wrap(~ species_id, scales = "free_y") + labs(x = "Weight (g)",
y = "Count")
ggsave("../outputs/surveys2.jpg")
```
...
# Simplify each species column
adelie <- adelie %>% mutate(Species = "Adelie")
chinstrap <- chinstrap %>% mutate(Species = "Chinstrap")
gentoo <- gentoo %>% mutate(Species = "Gentoo")
# Merge dataframes
mypenguins <- full_join(adelie, chinstrap)
# Pull year out of date and make year column
year_vect <- str_extract(mypenguins$`Date Egg`, "(....)-") %>% str_remove("-")
mypenguins <- mypenguins %>% mutate(year = year_vect)
# Make sex lowercase
mypenguins <- mutate(mypenguins, Sex = tolower(mypenguins$Sex))
# Delete unneeded columns and Rename columns
mypenguins <- select(mypenguins, species = Species,
       island = Island,
       ...
       year)
# Change . to NA
mypenguins <- mypenguins %>% mutate(sex = na_if(sex, ".")) # BONUS üps lol: change year to integer
mypenguins <- mypenguins %>% mutate(year = as.integer(year))
```

c.  Run the `setdiff()` function to make sure that your code worked (it shouldn't have any issues, but it is good to check!)
```{r}
# check
setdiff(mypenguins, penguins)
```

d.  Save your version of the cleaned penguins data into your clean data folder.
```{r}
write_csv(mypenguins, "../data_clean/mypenguins.csv")
```
