...
```{r}
# 2a
sqrts <- lapply(seq_list, sqrt)
do.call(rbind, sqrts) %>% head()
```


(b) Use `mclapply` (or `parLapply`!!) to do the same thing. Remember to set up the number of cores you will use, first.

```{r}
# 2b
clust <- parallel::makeCluster(ncores)
results <- parallel::parLapply(cl = clust, seq_list, sqrt)
head(results)
```
...
```{r}
source("../scripts/dino_allometry_fxn.R")
...
for (i in 1:length(dinos$species)) {
  masses[i] <- mass_from_length(length = dinos$lengths[i], dino_group = dinos$species[i])
}
head(masses)
```
...
```{r}
output <- foreach::foreach(i = 1:length(dinos$species), .combine = c) %do% {
  mass_from_length(dinos$lengths[i], dinos$species[i])
}
head(output)
```
...
output <- foreach::foreach(i = 1:length(dinos$species), .combine = c) %dopar% {
  mass_from_length(dinos$lengths[i], dinos$species[i])
}
head(output)
...
```{r}
# 4a
change_in_forest <- vector(mode = "numeric", length = length(forest$"2021"))

for (i in 1:length(forest$"2021")) {
...
```{r}
# 4b
output <- foreach(i = 1:length(forest$"2021"), .combine = c) %do% {
...
# 4c
doParallel::registerDoParallel(ncores)
output <- foreach(i = 1:length(forest$"2021"), .combine = c) %dopar% {
  forest$"2021"[i]-forest$"1990" [i]
}
