...
# 5d
head(pie_crab)
pie_crab %>% mutate(size_category = if_else(crab_size >= 20, "large", "small"))
```
...
```{r}
# 5e
pie_crab %>% mutate(size_category3 = if_else(size >= 20, "large",
        if_else(size >= 10 & size < 20, "medium", "small")))
```
