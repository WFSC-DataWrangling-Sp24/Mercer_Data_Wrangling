...
```{r}
# 5
convert_kg_to_pounds <- function(kilograms) {
...
count_data <- data.frame(state = state_vector, count = count_vector, area = area_vector, site = site_vector)
count_data
```
...
```{r}
# 6b
plot_6 <- function(df, color_by) {
  ggplot(data = df, aes(area, count, color = {{color_by}})) +
    geom_point(size = 3)
}

plot_6(count_data, state)

ggsave("../outputs/plot_6b.png")
```
...
```{r}
# 6c
plot_6(count_data, site)

ggsave("../outputs/plot_6c.png")
```
