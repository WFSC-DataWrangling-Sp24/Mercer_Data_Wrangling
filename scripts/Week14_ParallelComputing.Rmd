...
capt_histories <- mclapply(tags, create_capture_hist, data = ratdat_medium, mc.cores = ncores)
toc()
```
...
results <- parallel::parLapply(cl = clust, tags, create_capture_hist, data = ratdat_medium)
toc()
```
...
output <- foreach::foreach(i = 1:length(tags)) %do% {
  create_capture_hist(tags[i], ratdat_medium)
}
toc()
